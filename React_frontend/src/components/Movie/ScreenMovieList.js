...
const ScreenMovieList = () => {
  ...
  const [Limit, setLimit] = useState(screenmovieLimit); //더보기 리미트

  // 검색칸 내용 변수
  const [search, setsearch] = useState(screenmovieSearch);
  const handleSearchChange = e => {
    setsearch(e.target.value);
  };

  // 정렬 버튼 css 변수
	const [ratebutton, setratebutton] = useState(screenmovieSortRate);
	const [likebutton, setlikebutton] = useState(screenmovieSortLike);
 
  // 로그인 상태에 따라 전체 검색이 다름(좋아요 표시 때문)
  useEffect(() => {
    // 사용자가 브라우저에 있는 뒤로가기 또는 앞으로가기 버튼을 누른것을 감지
    if (screenmovieKey !== location.key) {
      dispatch({
    ...
    if (screenmovieId !== LOGIN_data.uid) {
      dispatch({
        type: SCREENMOVIE_REQUEST,
        data: {
          uid: LOGIN_data.uid,
  ...
  }, [screenmovieKey, screenmovieId, location, LOGIN_data, search, ratebutton, likebutton, Limit, dispatch]);
